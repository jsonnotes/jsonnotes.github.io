...
const { callProcedure, query_data, getNote, addNote } = require("../src/dbconn.ts");
...
const commands = {
  "add-note": async ([schemaHash, data]) => {
    ...
    console.log(`Note added with hash: ${JSON.parse(result)}`);
  },
  "new": async ([schemaRef]) => {
    if (!schemaRef) return console.error("Usage: npm run db new <schemaHash>");
    const schemaHash = schemaRef;
  ...
  "get-note": async ([ref]) => {
    console.log(JSON.stringify(await getNote(ref), null, 2));
  },
  "remote": async ([ref, arg = "null"]) => {
    const result = await callProcedure("run_note_async", { hash: ref, arg });
  ...
  "pull": async ([folder = "notes-backup"]) => {
    ...
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const notes = await query_data("select hash, schemaHash, data from note");
    notes.rows.forEach(([hash, schemaHash, data]) =>
      fs.writeFileSync(path.join(dir, `${hash}.json`), JSON.stringify({ hash, schemaHash, data }, null, 2))
    );
  ...
  "push": async ([folder = "notes-backup"]) => {
    ...
    for (const file of files) {
      ...
      if (!schemaHash) { console.log(`Skip ${file}: schemaHash not found`); skipped++; continue; }
      try {
